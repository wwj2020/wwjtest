...
object Test3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /*for(indexAtKey <- indexAtKeys){
      println("!!!!!!!!!!atKey"+indexAtKey)
    }
    for(indexDvKey <- indexDvKeys){
      println("!!!!!!!!!!dvKey"+indexDvKey)
    }*/

    val conf = new SparkConf()
      .setAppName(s"${Test3.getClass.getSimpleName}")
...
//            println("ttt索引表表名为：" + itnames.next())
          ...
          partition.foreach(jsonobj => {
            ...
            if (table.equals("acceptance")) {
              ...
              for (atKey <- basicAtKeys) {
                var atValue = ""
                if (!atKey.equals("atbillcode") && !atKey.equals("atcode")) {
                  if (dataObj.containsKey(atKey)) {
                    println(" fff基本表列为（json有数据）：" + atKey)
                    atValue = dataObj.getString(atKey)
                  }
                ...
                if (indexAtKeys.contains(atKey)) {
                  println((" fff获取的索引表列为：" + atKey))
              ...
              for (dvKey <- basicDvKeys) {
                var dvValue = ""
                if (!dvKey.equals("dvbillcode") && !dvKey.equals("dvcode")) {
                  if (dataObj.containsKey(dvKey)) {
                    println((" fff基本表列为(json有数据)：" + dvKey))
                    dvValue = dataObj.getString(dvKey)
                  }
                ...
                if (indexDvKeys.contains(dvKey)) {
                  println((" fff获取的索引表列为(空值)：" + dvKey))
                  ...
                  for (table <- indexTablesConnection) {
                    if (indexRelations(dvKey).equals(table.getName.toString)) {
                      println("   ttt要存的索引表为：" + table)
                      table.put(put1)
                    }
                  }
